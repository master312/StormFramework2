 ...
 * https://github.com/oxygine/oxygine-framework/blob/master/oxygine/src/math/Vector2.h */
...
public:
    ...
    friend std::istream &operator>>(std::istream& input, VectorT2<T>& obj) {
       input >> obj;
       return input;
    }
    ...
    T length() const {return (T)StormScalarMath::sqrt(x * x + y * y);}
...
{
    T norm = T(1.0) / StormScalarMath::sqrt(v.x * v.x + v.y * v.y);
    out = v;
    /*
    if (norm < 0.0001)
    {
