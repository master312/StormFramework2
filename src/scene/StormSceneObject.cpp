...
void StormSceneObject::fireEvent(SSceneObjectEventType type) {
    auto iter = _Observers.find(type);
    if (iter == _Observers.end()) {
        /* There are not any observers listening to events of specified type */
        return;
    }

    for (SSceneObjectEventObserver& observer : iter->second) {
        observer.callbackMethod(nullptr);
    }
}

void StormSceneObject::registerEventObserver(SSceneObjectEventObserver observer) {
    if (!observer.component || observer.type == S_OBSERVER_EVENT_UNDEFINED || !observer.callbackMethod) {
        LOG(ERROR) << "Tryed to attatch incoplete event observer to scene object";
        return;
    }

    _Observers[observer.type].push_back(observer);
    
    LOG(DEBUG) << "Observer of type " << observer.type << " added to scene object";
}

void StormSceneObject::removeEventObserver(SSceneComponent* component, SSceneObjectEventType type /* = S_OBSERVER_EVENT_UNDEFINED */) {
    if (type == S_OBSERVER_EVENT_UNDEFINED) {
        /* Remove all observers by @component */
        for (auto& iter : _Observers) {
            removeEventObserverFromVector(component, iter.second);
        }
    } else {
        /* Remove observer of specific type */
        auto iter = _Observers.find(type);
        if (iter == _Observers.end()) {
            return;
        }
...
void StormSceneObject::removeEventObserverFromVector(SSceneComponent* component, std::vector<SSceneObjectEventObserver>& vector) {
    for (size_t i = 0; i < vector.size(); i++) {
        SSceneObjectEventObserver* observer = &vector[i];
        if (!observer->component || observer->component != component) {
            continue;
        }
