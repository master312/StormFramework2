...
SSystemSprite::SSystemSprite(SScene* scene) : SSceneComponentSystem(scene) {
    _Type = S_SCENE_OBJECT_COM_SPRITE;
}

SSystemSprite::~SSystemSprite() {
}

void SSystemSprite::tick(float deltaTime) {
    for (size_t i = 0; i < _SpriteComponents.size(); i++) {
        SComSprite* sprite = _SpriteComponents[i];
...
void SSystemSprite::render(StormRenderer* renderer) {
...
void SSystemSprite::onComponentAdded(SSceneComponent* component) {
    if (!validateComponent(component, S_SCENE_OBJECT_COM_SPRITE)) {
        return;
    }
    SComSprite* com = static_cast<SComSprite*>(component);
...
void SSystemSprite::onComponentRemoved(SSceneComponent* component) {
    for (size_t i = 0; i < _SpriteComponents.size(); i++) {
...
SComSpriteSheet* SSystemSprite::getSpriteSheet(const std::string& filename) {
...
int SSystemSprite::loadSpriteSheetFromXml(spStormResourceFile file) {
    if (!file) {
        LOG(ERROR) << "SSystemSprite::loadSpriteSheetFromXml Resource file is nullptr";
        return -3;
    }
...
void SSystemSprite::tickSpriteAnimation(SComSprite* sprite, float deltaTime) {
...
void SSystemSprite::renderSprite(SComSprite* sprite, StormRenderer* renderer) {
...
void SSystemSprite::sortComponentsByZ() {
    static struct sortStruct {
        bool operator() (SComSprite* i, SComSprite* j) {
